...
## 阶段二补充 · 报表增强项（2026-07-17 记录）

> 背景：当前「报表」主链路已覆盖支出 / 收入 / 结余、周 / 月 / 年 / 自定义、分类下钻、成员下钻等核心图表与分析。以下为产品说明书 V1 中仍需后续补齐的增强项，暂不阻塞主报表体验验证。

- [ ] **#R1 全局筛选**：成员 / 账户 / 分类 / 标签筛选 Sheet；筛选条件需统一作用于摘要、趋势、构成、下钻与洞察，底部显示命中流水数，支持重置与应用。
  - 依赖：若「账户 / 标签」尚未进入流水数据模型或记账入口，需要先补数据字段与录入 / 编辑逻辑。
- [ ] **#R2 财务洞察**：异常支出、预算风险、月末结余预测、目标进度预测、行动建议；要求洞察卡解释“为什么”和“下一步怎么做”，避免只给数字。
  - 依赖：至少需要足够历史周期数据；规则阈值需可配置或集中管理。
- [ ] **#R3 更多统计**：年度热力图、工作日 / 周末消费节奏、星期分布、数据完整度等；默认收纳到「更多统计」，避免主报表过载。
- [ ] **#R4 收入目标与目标预测**：收入页补年度 / 自定义收入目标、主动 / 被动收入目标进度、预计完成情况。
  - 依赖：是否新增收入目标模型，或先以前端本地 / 偏好配置承载。
- [ ] **#R5 图表交互与可访问性**：点按柱 / 点 / 扇区显示精确值；为图表提供文本摘要；颜色之外增加纹理 / 标签辅助；完善 VoiceOver 描述。
- [ ] **#R6 报表卡片管理 UI**：显隐 + 排序子页；存在隐藏卡时，报表底部显示「添加数据卡片」入口。数据字段已在 `accounting_preferences.report_card_order/report_card_hidden` 预留。

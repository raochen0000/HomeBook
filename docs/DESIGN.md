...
> 文档版本：v0.5.2（**新增 §5.10 个人设置 / 账号与安全页**（流程 15）：「我的」顶部用户信息块点击进入的账号页 UI 规格——头像 / 昵称、手机 / 邮箱 / Apple 绑定换绑解绑、修改密码、账号注销；主体 L-Native（Form/Section），退出登录仍留「我的」页底部。详见下「v0.5.2 变更摘要」。历史 v0.5.1：首页 hero 脉搏卡 + 月度总结全屏可翻月；v0.5.0：去品牌温度、回归 iOS 26 原生中性风格）
> 最后更新：2026-07-01
> 关联文档：PRD.md（v0.1.7；视觉规范对应 §22，账号页见流程 15 §17）、IA.md、MVP.md、DATAMODEL.md、TECH.md
> 负责人：产品组 / 设计 / 客户端
> 用途：作为「家账」App 视觉与组件实现的单一事实来源（Single Source of Truth）。

---

## v0.5.2 变更摘要（相对 v0.5.1）

**新增「个人设置 / 账号与安全页」UI 规格（§5.10）**，对应 PRD 流程 15（§17）：「我的」Tab 顶部用户信息块点击进入的统一账号页。

1. **入口收敛**：顶部用户块 + 原「我的」列表里「个人信息 / 账号与安全 / 绑定手机号」三处重复入口合并为一页。
2. **主体 L-Native**：优先原生 `Form` / `Section` / `TextField` / `SecureField` / `Menu` / `ConfirmationDialog`（§5.6）；仅 OTP 输入、滑动确认、头像上传预览复用既有自定义件，不新造。
3. **登录方式行**：手机 / 邮箱 / Apple 按「已绑 / 未绑」态给行尾操作；**账号至少保留一种登录方式**——仅剩一种时该行解绑置灰（安全底线，PRD §17.4）。
4. **危险操作分区**：账号注销为 `state/danger` 行 + `ConfirmationDialog`（destructive）+ 手机号后 4 位 + 滑动确认；**退出登录不进本页**，仍留「我的」页底部（登出 ≠ 销号）。

---

## v0.5.1 变更摘要（相对 v0.5.0）

**主旨：让首页与报表各自回答各自的问题**——首页回答「本月还好吗（当下）」，报表回答「钱怎么变（跨周期）」，消除两张概览卡的重复感与首页内部「红条说超支、卡片说结余为正」的口径打架。

1. **首页 hero 重构为「本月脉搏卡」**（新增 §5.9）：本月概览卡 + 独立超支条幅 → **合并为一张预算口径的脉搏卡**（本月预算 / 已用 / 剩余可支配 + 进度条 + 距月底 X 天），**锁本月、无时间切换**；卡内次级一行为现金流口径的「本月结余」单数字（中性色、不带环比）。
2. **超支预警内联**（§5.8）：80%/超支预警由独立顶部条幅改为**脉搏卡的 warning/danger 态**（进度条变色、剩余翻转为「已超支 ¥X」），**§5.8 不再保留独立超支浮条**。
3. **月度总结改全屏可翻月**（§12）：由报表底部 sheet 的上月静态卡 → **首页 hero 整卡点击进入的全屏、可翻月视图**，默认本月至今（实时 · 进行中、不存图），上月及更早为已结算仪式实例（保留暖心文案 + 保存图片）；报表 Tab 内不再设入口。
4. **周期切换归报表独占**：周 / 月 / 年切换 + 环比 + 结余率为报表锚点；首页脉搏卡不提供任何周期切换。
...
| 元素             | 实现                                                                                           |
| ---------------- | ---------------------------------------------------------------------------------------------- |
| 顶部条幅 Banner  | 自定义层（RN）或原生 `Overlay`；底色用中性 `bg/card`。**v0.5.1：预算 80%/超支预警不再走独立顶部浮条，改为内联进首页 hero「本月脉搏卡」的 warning/danger 态（见 §5.9）**；本条幅仅用于「月初『上月总结来啦』轻提醒」等非预警通知（中性底） |
...
### 5.9 首页 hero「本月脉搏卡」（v0.5.1 新增）

> 首页概览区由「本月概览卡 + 独立超支条幅」合并为单张 hero 卡，回答「本月还好吗、还能花吗（当下）」。**锁本月、无任何时间切换**（周期切换归报表，§5.7 / PRD §11.6）。整卡为可点容器：**点击 → 全屏「月度总结」，默认本月至今**（§12 / PRD §11.5.2）。

**已设预算态（主体 = 预算口径）**

| 部位             | 实现                                                                                                                                  |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 卡容器           | 原生卡（`bg/card` + `radius/lg` + `space/4` 内边距），整卡 `Button`（`buttonStyle('plain')`）→ 全屏月度总结；右上角 `chevron` 暗示可进入 |
| 主数字           | 「剩余可支配 = 总预算 − 已用」`type/title1`；danger 态翻转为「已超支 ¥X」                                                             |
| 进度条           | `LinearProgress` 自定义变色（§5.5）：`<80%` → `accent`；`≥80%` → `state/warning`（琥珀）；`>100%` → `state/danger`（红）              |
| 辅助行           | 「本月预算 ¥X · 已用 ¥Y · 距月底 N 天」`type/footnote`、`text/secondary`                                                              |
| 现金流结余行     | 卡内次级一行：「本月结余 = 收入 − 支出」**单数字**，`text/primary` 中性色，**不带环比角标**（环比归报表）                              |

**降级 / 异常态**

| 场景             | 处理                                                                                                                                |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 未设预算         | 退回**现金流摘要态**：主数字「本月结余」（`text/primary`）+ 下方「支出」（`semantic/expense`）「收入」（`semantic/income`）小数字，均不带环比；附「设置预算」引导（仅户主可点，普通成员显只读「待户主设置预算」）。**仍锁本月、无时间切换** |
| 本月无记账       | 空状态文案 + 「记一笔」CTA；已设预算则进度显示 0% 已用                                                                                |
| 上月总结不可用   | 月初「上月总结来啦」轻提醒**隐藏**，不置灰占位（整卡点击仍可进本月至今总结）                                                          |
| 加载失败         | 保留上次缓存数据 + 轻提示重试，口径与报表一致                                                                                        |

> **预警内联**：80%/超支不再用独立顶部红条（§5.8），统一由本卡的进度条变色 + 主数字翻转表达，避免「红条说超支、卡片说结余为正」的口径打架。

### 5.10 个人设置 / 账号与安全页（流程 15，v0.5.2 新增）

> 「我的」Tab 顶部用户信息块点击进入的账号页（PRD 流程 15 / §17）。承载个人资料（头像 / 昵称）、登录方式（手机 / 邮箱 / Apple 的绑定 / 换绑 / 解绑）、修改密码、账号注销。**主体为 L-Native**：优先原生 `Form` / `Section` / `List`，仅 OTP 输入、滑动确认、头像上传预览复用既有自定义件。**退出登录不在本页**（留「我的」页底部，§5.2 / IA §5）。两条安全底线：**账号至少保留一种登录方式**、**敏感操作先身份校验**（PRD §17.4）。

**页面骨架**

| 部位     | 实现                                                                                     | 说明                          |
| -------- | ---------------------------------------------------------------------------------------- | ----------------------------- |
| 页面外壳 | Expo Router Stack push（`NavigationStack`），标题「账号与安全」                          | 从「我的」顶部块 push，系统返回 |
| 分区容器 | 原生 `Form` + `Section`（header / footer）                                               | 系统 grouped 分组背景 + 分隔，免自绘卡 |
| 头像行   | `Section` 首行：圆形 `Image`（`clipShape('circle')` + `frame` 64–72）+ 相机角标；点击走既有上传链路（§9.2） | 复用 `mine.tsx` 头像上传      |
| 昵称行   | `TextField`（或点击进编辑 sheet）+ 末尾 `chevron.right`                                  | `type/body`                   |

**登录方式区（`Section` header：「登录方式」）**

每种登录方式一行，行尾按状态给操作：

| 行             | 已绑态                       | 未绑态           | 行尾操作                                          |
| -------------- | ---------------------------- | ---------------- | ------------------------------------------------- |
| 手机号 `iphone`    | 脱敏号（`138 **** 5678`，等宽 monospaced） | 「未绑定」`text/tertiary` | 已绑 →「更换」；未绑 →「绑定」（`Button` plain，`accent`） |
| 邮箱 `envelope`    | 脱敏邮箱                     | 「未绑定」       | 已绑 →「更换 / 解绑」（`Menu`）；未绑 →「绑定」    |
| Apple `apple.logo` | 「已连接」                   | 「未连接」       | 已绑 →「解绑」；未绑 →「连接」                     |

- **解绑置灰规则**：当前仅剩一种登录方式时，该行解绑操作 `opacity/disabled` + `Section` footer 提示「请先绑定其它登录方式」（PRD §17.4）。
- 手机号**不提供解绑**，仅「绑定 / 更换」（PRD §17.6）。

**密码区（`Section` header：「密码」）**

| 场景     | 行文案                                       | 实现                                                |
| -------- | -------------------------------------------- | --------------------------------------------------- |
| 已设密码 | 「修改密码」                                 | 进编辑：`SecureField` ×（旧 / 新 / 确认），身份校验后保存 |
| 未设密码 | 「设置密码」；无绑定邮箱时置灰 + footer「需先绑定邮箱」 | 同上，密码依附邮箱登录方式                           |

**危险操作区（独立 `Section`，置页底）**

| 元素     | 实现                                                                                                       |
| -------- | -------------------------------------------------------------------------------------------------------- |
| 账号注销 | 行文字 `state/danger`（红），点击进注销流程（PRD 流程 12 / §14）；二次确认走 `ConfirmationDialog`（`role:'destructive'`）+ 输入手机号后 4 位（自定义）+ 滑动确认（§5.8） |

> **退出登录**不在本页：仍在「我的」页底部独立按钮（`state/danger` 文字、整行 `bg/card`），与「账号注销」层级区分——登出 ≠ 销号（§5.2 / IA §5）。

**身份校验 / OTP 子流程（复用件，不新造）**

| 元素             | 实现                                                                                     |
| ---------------- | ---------------------------------------------------------------------------------------- |
| 手机 / 邮箱输入  | 原生 `TextField`（`keyboardType` 对应），`accent` 主 CTA 发码                            |
| OTP 验证码输入   | 复用登录页 OTP 输入件（自定义层）；换绑先旧号 OTP、再新号 OTP（PRD §17.6）                |
| 敏感操作二次确认 | `ConfirmationDialog` / 输入后 4 位 + 滑动确认（与转让 / 移除同款，§5.8 / PRD §7、§17.4） |
| 唯一性 / 错误反馈 | 行内 `state/danger` 文案（「已被其它账号使用」「验证码错误」）；校验不通过不改绑          |

**落地小结**

- 全页优先 L-Native（`Form`/`Section`/`TextField`/`SecureField`/`Menu`/`ConfirmationDialog`，见 §5.6）；自定义件仅限 OTP 输入、滑动确认、头像上传预览，均已在登录 / 记账 / 转让流程中存在。
- **离线**：整页「需联网」兜底（PRD §17.4），顶部 `state/info` 轻提示，操作按钮禁用。
...
| 月度总结生成（§11） | 月初「上月总结来啦」轻提醒（中性底，前 7 天）→ 点击进**全屏月度总结**（深链定位上月已结算实例）；常态入口＝首页 hero 脉搏卡整卡点击（默认本月至今） | 文案池随机（PRD §11.8）   | 自定义层 |

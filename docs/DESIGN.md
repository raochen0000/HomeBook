...
> 文档版本：v0.3.1（在 v0.3 native-first 基础上，敲定底部 Tab Bar 三状态与记账面板呈现方式）
> 目标版本：v0.1（App 发布版本，以 PRD.md 为准）
> 最后更新：2026-06-06
> 关联文档：PRD.md（§22）、IA.md、MVP.md、DATAMODEL.md、TECH.md
> 负责人：产品组 / 设计 / 客户端
...
| `bg/base`        | systemGroupedBackground          | `#F2F2F7`             | `#000000`               | 页面底              |
...
> 原生 `Form`/`List`/`Section` 会自动套用 grouped background 与分隔线，**多数情况无需手动指定**；次要文本优先用 `foregroundStyle` 的 hierarchical `secondary` 而非硬编码灰度。
...
> **v1 首发固定 Cool**（iOS 原生观感）。注意：原生 `Form`/`List` 的分组底色由系统语义色驱动，Warm 变体在原生件上**只能整体覆盖 `bg/base`（如用自定义 `ScrollView` 背景）**，对系统 grouped 背景的改写有限；故 Warm 暂定仅作用于自定义层与可控背景，作为远期完善项。
...
### 5.2 底部 Tab Bar（定稿：iOS 26 原生 Liquid Glass，三状态，全程对齐 Apple 播客）

**实现**：用 Expo Router **`NativeTabs`**（`expo-router/unstable-native-tabs`），渲染交给系统，iOS 26 自动得到 Liquid Glass，无需自管材质（背景由系统按内容自适应、无法覆盖，与 §3 一致）。

**结构（四个角色）**

```
药丸（左）   ：首页（house）  报表（chart.pie）  家庭（person.2）  — 3 个导航 Tab
独立圆（右） ：🔍 搜索        — role="search"，原生独立圆，点击进入搜索
记一笔       ：BottomAccessory 三栏条，常驻所有 Tab，整条可点 → 弹大号 detent Sheet（§5.3）
我的         ：顶栏右上角头像入口（§5.1），不进底部
```
...
| 状态                      | 布局                                                                                                                  |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| ① 常态（未滚动）          | 上＝记一笔三栏条（满宽）；下＝药丸(首页/报表/家庭) + 右侧 Search 独立圆。**两行堆叠**                                 |
| ② Search 激活（键盘弹起） | 药丸**变形为搜索输入框** + 左侧返回圆（＝进入前所在 Tab 的图标，点击回原 Tab）；**记一笔条隐藏**；底部＝搜索框 + 键盘 |
| ③ 滚动（tab bar 最小化）  | 合并为**一行**：[当前 Tab 图标] + [记一笔收缩条：配图 + 截断格言 + ➕] + [Search 圆]，Tab label 收起                  |

> **记一笔条 = 完全照搬播客 mini-player 的行为**：一直常驻（不分有无内容），**仅在「②Search 激活」时隐藏**。格言始终展示，收缩态**可截断**（不隐藏）。

**记一笔三栏条尺寸**

| 形态         | 高度  | 三栏                                                                                  |
| ------------ | ----- | ------------------------------------------------------------------------------------- |
| 展开（①②外） | 56–64 | 配图 `40×40`（圆角 md）／格言（弹性、单行、`↔` 横向循环）／➕ 圆（视觉 36，热区 ≥44） |
| 收缩（③）    | ~38   | 配图缩到 `24`／格言截断（marquee 暂停）／➕ 缩到 28；整条仍可点                       |

**实现要点**

| 项                 | 做法                                                                                |
| ------------------ | ----------------------------------------------------------------------------------- |
| 三个导航 Tab       | `NativeTabs.Trigger` + `Trigger.Icon sf="..."` + `Trigger.Label`                    |
| 右侧独立 Search 圆 | `NativeTabs.Trigger role="search"`（原生独立圆 + 药丸变搜索框 + 返回圆，全免费）    |
| 记一笔条           | `NativeTabs.BottomAccessory` 内放三栏（RN 内容）；整条 `onPress` → 弹 Sheet（§5.3） |
| 搜索内容页         | **MVP 先占位 ToDo**：简单实现「搜流水 / 分类 / 成员」，后续迭代增强（见 §16）       |
| 格言 marquee       | 自定义（Reanimated）；**减弱动态下停滚 + 截断**；VoiceOver 读全文                   |
| 暗色闪烁修复       | 用 `ThemeProvider`（expo-router）传入匹配深色主题，避免切换时玻璃闪烁               |

> ⚠️ **最高实现风险，spike 优先**：`NativeTabs` 为 **alpha / unstable API**。上线前先验证「记一笔 BottomAccessory 三栏 + Search 三状态联动 + 整条点击弹 Sheet」整套跑通；`role="search"` 槽位有已知 Badge 裁切 bug（家账不用 Badge，不受影响）。
...
| **底部「今日格言」条**        | 与 §5.2 记一笔三栏条**同源同款**（同一条格言）；marquee 规则一致（减弱动态停滚、可截断、VoiceOver 读全文） |

> 区域对应：主内容区＝记账主体（金额/分类/字段/保存）；底部条＝今日格言（与小条同源）。这是「原生壳 + 自定义内容」的标杆案例，其它需自定义输入的 Sheet 复用同一模式。
> 「格言从小条原地位移展开进全屏」的共享元素转场 = **远期迭代**（§16）；首发用「同源 + 淡入」即可，不为真·位移动画卡住。

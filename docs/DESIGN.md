...
> 文档版本：v0.5.1（**首页 hero 概览重构**——本月概览卡 + 独立超支条幅合并为「本月脉搏卡」（§5.9）：预算口径 · 锁本月 · 80%/超支预警内联 warning/danger 态（§5.8 不再设独立超支浮条）+ 现金流结余行 + 月度总结全屏入口；月度总结由报表底部 sheet 改为首页 hero 整卡点击进入的全屏可翻月视图（§12）。详见下「v0.5.1 变更摘要」。历史 v0.5.0：去品牌温度、回归 iOS 26 原生中性风格）
> 最后更新：2026-06-27

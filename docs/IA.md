...
**G2 · 账号与安全页** — Push · ⬜
- 目的：个人资料 + 登录方式 + 密码 + 注销总览。
- 区块（上→下）：个人资料（头像 / 昵称 → G3）→ 登录方式分组（手机 / 邮箱 / Apple 各一行，显「已绑 / 未绑」+ 操作）→ 密码（修改 / 设置）→ 危险操作（账号注销 → G9）。
- 关键态：仅剩一种登录方式时该行解绑置灰 + footer 提示 / 各行已绑 vs 未绑。
- 操作：进各登录方式子流程 G4-G7、改密 G7、注销 G9。
...
**G4 · 手机号 绑定 / 换绑** — 子流程 · ⬜（底层 auth 已有）
- 区块：未绑＝输入手机号 → 获取 OTP → 验证 → 绑定；已绑换绑＝旧号 OTP 校验身份 → 输入新号 → 新号 OTP → 完成。
- 关键态：OTP 倒计时 / 错误过期重发 / 新号被占用（阻断）/ **不提供解绑**。
- 关联：PRD §17.6。

**G5 · 邮箱 绑定 / 换绑 / 解绑** — 子流程 · ⬜
- 区块：绑定＝邮箱 + 设密码 → 验证 → 绑定；换绑＝身份校验 → 新邮箱验证 → 换；解绑＝校验「至少留一种」→ 身份校验 → 解绑。
- 关键态：邮箱被占用（阻断）/ 仅剩此一种时解绑置灰。
- 关联：PRD §17.6。

**G6 · Apple 绑定 / 解绑** — 子流程 · ⬜
- 区块：绑定＝Apple 授权 → 合并到当前账号；解绑＝校验「至少留一种」→ 身份校验 → 解绑。
- 关键态：仅剩此一种时解绑置灰。
- 关联：PRD §17.6。

...
> 文档版本：v0.3.0（海外首版登录与账号信息架构调整为邮箱密码 + Apple，移除手机号登录/管理页，敏感操作统一走邮箱或 Apple 近期重新认证）
> 最后更新：2026-08-31
...
```mermaid
flowchart TD
    Start([启动]) --> Login[登录页<br/>邮箱密码 + Apple<br/>登录即注册]
    Login --> Home
    ...
    Mine --> Account[账号与安全<br/>头像·昵称 / 邮箱·Apple 管理小页<br/>修改密码 / 账号注销]
...
| Tab 我的          | 顶部用户信息块 → **账号与安全**页（头像 / 昵称、邮箱 / Apple 各自的管理小页、修改密码、账号注销；PRD 流程 15）；三张设置卡——**记账与数据**（记账设置 / 导出数据）、**通用**（通知设置 / 深色模式 / 语言）、**帮助与关于**（帮助中心 / 意见反馈 / 关于家账），规格见 PRD 流程 16；**退出登录**独立留页底 | 流程 15 / 16 / 12      | 账号页已实现；待移除手机号入口并补近期重新认证 |
...
- 「我的」Tab 顶部用户信息块点击进入**账号与安全**页（PRD 流程 15，§17），统一承载头像 / 昵称、邮箱 / Apple 管理、修改密码与账号注销；登录方式行整行 push 进管理小页。两条安全底线：**账号至少保留一种登录方式**、**敏感操作先通过邮箱或 Apple 近期重新认证**。**退出登录**不进该页，保持在「我的」页底部独立按钮（登出 ≠ 销号）。
- 「我的」列表按三张卡分组（**记账与数据 / 通用 / 帮助与关于**），承接原占位入口的实功能，规格见 PRD 流程 16（§18）：深色模式 / 语言用**行内下拉菜单**即时切换，记账设置 / 导出数据 / 通知设置 / 帮助 / 反馈 / 关于走**子页**，用户协议 / 隐私政策为**内置页 + 底部 Sheet**（合规必备，不外链）。个人级偏好只影响本人；账期 / 币种等家庭级设置不入本页。
...
- 目的：邮箱密码 / Apple 登录，登录即注册、无独立注册页。
- 区块（上→下）：品牌头（Logo + 名，无插画）→ 邮箱 + 密码 → 登录/注册主按钮 → Apple 登录按钮 → 底部协议勾选行《用户协议》《隐私政策》。
- 关键态：邮箱未确认 / 登录失败 / SMTP 延迟或不可用 / Apple 取消或失败。
- 操作：主＝邮箱登录/注册；次＝Apple、忘记密码、点协议链接（→ B2）。
- 关联：PRD §3、§3.6。
...
- 目的：手输 / 扫码 → 预览确认 → 加入。
- 区块（上→下）：输入区（6 段码框 ｜「改用扫码」）→ 家庭预览卡（封面 / 名 / 户主昵称头像 / 成员头像堆叠 + X·5 人）→ 加入影响提示条 → 加入按钮。
...
- 目的：破坏性操作的强确认闸门。
- 区块（上→下）：警示文案 → 文字匹配输入闸门（转让 / 移除＝对方昵称；解散＝输入家庭名）→ 滑动以确认。
- 关键态：闸门不匹配（禁用）/ 处理中 / 失败重试 / 转让成功追问「是否顺便退出」。
- 关联：PRD §7.4 / §8.4（产品文档与代码统一使用对方昵称）。
...
- 区块：最新 100 条待处理通知（被移除 / 转让 / 继任 / 目标达成 / 预算预警 / 月度总结）；点按阅读后删除，可一键清除当前列表。
- 关键态：空态 / 有待处理红点 / 点条目阅读后删除。
- 关联：PRD §15（流程 13）。
...
- 目的：账号入口 + 分组设置 + 退出。
- 区块（上→下）：用户信息块（头像 + 昵称 + 邮箱/Apple 状态 → 点击进 G2）→ 卡一「记账与数据」（记账设置 / 导出数据）→ 卡二「通用」（通知设置 / 深色模式〔行内下拉〕/ 语言〔行内下拉〕）→ 卡三「帮助与关于」（帮助中心 / 意见反馈 / 关于家账）→ 页底退出登录。
...
- 区块（上→下）：个人资料（头像 / 昵称 → G3）→ 登录方式分组（邮箱 / Apple 各一行，行尾只显状态值 + chevron，整行 push 进管理小页 G4-G5）→ 分组 footer（两条安全底线说明）→ 密码（修改 / 设置 → G6）→ 危险操作（账号注销 → G8，独立危险卡 + footer）。
- 关键态：各行已绑 vs 未绑（状态值文案差异）。
- 操作：进管理小页 G4-G5、改密 G6、注销 G8。
...
**G4 · 邮箱管理小页** — Push（自 G2 整行点击）· 🟡

- 区块：当前状态头（脱敏邮箱 / 未绑定）→ 操作（未绑＝“绑定”；已绑＝“更换”，首版不单独解绑）→ 近期重新认证 → 新邮箱验证。
- 关键态：邮箱被占用（不泄露账号枚举信息）/ 邮件延迟 / OTP 错误或过期。
- 关联：PRD §17.6。

**G5 · Apple 管理小页** — Push（自 G2 整行点击）· 🟡

- 区块：当前状态头（已连接 / 未连接）→ 操作（未绑＝“连接”；已绑＝“解绑”）→ 对应流程：绑定＝Apple 授权 → 合并到当前账号；解绑＝校验至少已有邮箱 → 近期重新认证 → 解绑。
- 关键态：仅剩 Apple 时“解绑”置灰 / Apple 授权取消或失败。
- 关联：PRD §17.6。

**G6 · 修改 / 设置密码** — 子流程 · 🟡

- 区块：已设＝近期重新认证（当前密码或邮箱 OTP）→ 新密码 → 保存；未设（仅 Apple）＝显示“设置密码”，需先绑定并确认邮箱。
- 关键态：无邮箱时引导先绑邮箱。
- 关联：PRD §17.6。

**G7 · 敏感操作近期重新认证** — 态 / Modal · ⬜

- 目的：换绑邮箱 / 解绑 Apple / 改密 / 注销前统一重新证明身份。
- 区块：邮箱账号＝当前密码或邮箱 OTP；仅 Apple 账号＝fresh Apple 授权 → 服务端验证新鲜度 → 通过 / 阻断。
- 关键态：校验失败、超时或过期均阻断该操作。
- 关联：PRD §17.4。

**G8 · 账号注销** — Push / 流程 · 🟡

- 目的：合规自助销号，不破坏「数据归家」。
- 区块（上→下）：身份判定说明（多人户主＝阻止，引导先转让 / 解散 ｜ 单人户主＝解散家庭 + 删号 ｜ 成员＝退出家庭〔历史保留〕+ 删号）→ 近期重新认证 → 输入“我确认注销”+ 滑动确认 → 执行 → 登出回登录页 + 登录方式释放说明。
...
- 关键态：选中即时切换界面语言 + 本机持久化；无存档时非中文设备默认 English。
- 关联：PRD §18.3.5。

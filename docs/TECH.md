...
> 文档版本：v0.2.0（后端方案落地：国内优先 → 阿里云境内托管；新增 §7 后端架构、区域适配层、合规与全球化演进；同步 DESIGN v0.3.1）
> 最后更新：2026-06-07
> 关联文档：PRD.md（v0.1.1，对应 §23）、DESIGN.md（v0.4.0）、IA.md（v0.2.0）、MVP.md（v0.1.2）、DATAMODEL.md（v0.1）、AGENTS.md（AI 编码业务铁律，根目录）
> 负责人：产品组 / 研发
...
| 路由 / 导航     | **expo-router** + **NativeTabs**（`unstable-native-tabs`）     | 文件式路由；原生 Tab Bar，对齐 IA §2 / DESIGN §5.2：药丸 3 Tab + 右侧 Search 圆 + 记一笔 BottomAccessory 三栏条（iOS 26 Liquid Glass） |
...
| ------------------ | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| **M0 地基**        | 脚手架 + 设计令牌主题 + 本地 DB + API 层骨架                                    | TS 主题（Light/Night）、NativeTabs（药丸 3 Tab + Search 圆 + 记一笔 BottomAccessory 三栏条）、搜索占位页、`<Money/>`、空状态 | 阿里云 Supabase 基座搭起（A1/A2 定夺）、迁移骨架 + 核心表 + RLS、CI 跑 pgTAP |
| **M1 账号 + 记账** | 流程 1 登录（手机 / Apple）、流程 2 记一笔、流程 10 编辑 / 删除                 | 记账 Sheet（大金额输入）、流水列表（按日分组 + 左滑）、离线同步队列                                                          | 阿里云短信 OTP + Supabase Auth、流水 RPC、WatermelonDB 同步函数（pull/push） |
...
| **M3 基础报表**    | 流程 9 基础版（本月收支结余 + 分类占比环形图）                                  | Victory Native 环形图 + 分类明细下钻                                                                                         | 报表聚合视图/RPC（排除储蓄类流水口径）                                       |
| **M4 增值（P1）**  | 分类管理 → 预算 → 储蓄目标 → 完整报表 / 月度总结 → 移除成员 → 通知体系          | 进度条 / 目标卡 / 庆祝动效、Banner、条形 / 折线 / 双柱图                                                                     | 储蓄存取 RPC、pg_cron（预算重置/月度总结/继任判定）、阿里云移动推送 EMAS     |
...
3. **合规前置**：ICP 备案、企业实名、短信签名报备有审核周期，**提前办理**，否则阻塞 M1 登录。

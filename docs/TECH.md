...
| **M4 增值（P1）**  | 分类管理 → 预算 → 储蓄目标 → 完整报表 / 月度总结 → 移除成员 → 通知体系          | 进度条 / 目标卡 / 庆祝动效、Banner；报表完整版：成员参与度（原生横向条）/ 发生额折线 / 累计同期双线 / 收支双柱 / 分类环比 / 大额 Top N 列表 / 结余率仪表 / 月度总结卡 | 储蓄存取 RPC、pg_cron（预算重置/月度总结/继任判定）、阿里云移动推送 EMAS、报表聚合扩展（分类环比 / 同期累计 / Top N 聚合） |

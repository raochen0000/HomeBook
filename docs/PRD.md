...
> 文档版本：v0.1.4（流程 3/4 调整：邀请新增「6 位文本邀请码」与二维码并列、文本码 3+3 分段展示 + 一键复制；加入家庭支持「手输码 / 扫码」统一确认面，满码自动拉取家庭预览卡（封面 + 户主显名 + 成员头像堆叠 + 人数）后再加入。流程 1 新增 §3.5 家庭信息（名称 / 封面）设置：新用户可选「完善家庭」步骤 + 户主在家庭设置页修改）
> 最后更新：2026-06-21
> 文档状态：已完成核心流程定义
> 负责人：产品组
...
| 引导页屏数       | 3 屏                                                                                                    |
| 是否可跳过引导   | ✅ 可跳过                                                                                               |
| 登录方式         | 手机号验证码 + 第三方登录（**MVP = 邮箱密码 + Apple；手机号 OTP 移至发布前**，详见 MVP.md §2.1 / §2.4） |
| 新用户家庭创建   | 自动创建单人家庭                                                                                        |
| 家庭信息完善     | 新用户**可选**步骤（家庭名 + 封面），可跳过；跳过则用默认家庭名 + 默认封面占位（见 §3.5）               |
| 是否引导加入家庭 | ❌ 不强制，用户可后续邀请或被邀请                                                                       |

### 3.5 家庭信息设置（名称 / 封面）

家庭名与封面用于家庭页头、加入家庭预览卡（流程 4）等处展示。设置分两处：

- **创建侧（新用户可选步骤）**：注册自动创建单人家庭后，提供一个**可跳过**的「完善家庭」步骤——设置家庭名 + 封面。跳过则用默认家庭名与默认封面占位（中性灰底，DESIGN §5.6），不阻断进入主页。
- **修改侧（家庭设置页）**：「家庭」Tab 内提供**家庭设置**入口，**仅户主**可修改家庭名与封面；普通成员只读。

| 决策点            | 选择                                                          | 理由                             |
| ----------------- | ------------------------------------------------------------- | -------------------------------- |
| 封面是否必填      | 否；无封面用默认占位（中性灰底）                              | 不阻断新用户上手                 |

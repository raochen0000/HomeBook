...
> 文档版本：v0.1.6（流程 8/9 首页概览重构：本月概览卡 + 独立超支条幅 → 合并为首页 hero「本月脉搏卡」（预算口径 · 锁本月 · 80%/超支预警内联 warning/danger 态，不再用独立顶部红条）；月度总结升级为首页 hero 整卡点击进入的**全屏、可翻月**视图（默认本月至今实时计算，上月及更早为已结算仪式实例），报表 Tab 内不再设月度总结入口。见 §10.4/§10.6/§11。历史：v0.1.5 手机号 OTP 升为主登录方式）
> 最后更新：2026-06-27
> 文档状态：已完成核心流程定义
> 负责人：产品组

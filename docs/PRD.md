...
> 文档版本：v0.1.7（新增**流程 15「个人设置（个人资料 + 账号与安全）」**：「我的」页顶部用户信息块点击进入统一账号页，含头像 / 昵称、手机号 / 邮箱 / Apple 登录方式的绑定 / 更换 / 解绑、修改密码、账号注销；确立「账号至少保留一种登录方式」「敏感操作先身份校验」两条安全底线；**账号注销（原 §14 流程 12）由远期落地为本页真实入口**，明确注销后手机号 / 邮箱 / Apple 释放、同号再登录为全新账号。见 §17。历史：v0.1.6 首页脉搏卡 + 月度总结全屏可翻月；v0.1.5 手机号 OTP 升为主登录方式）
> 最后更新：2026-07-01
> 文档状态：已完成核心流程定义
> 负责人：产品组

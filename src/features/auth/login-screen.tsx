 ...
 *
 * **当前阶段（2026-06-27）：手机号 OTP 入口已重新展示**（手机号为主 + 邮箱 / Apple 为次），
 * 但短信通道仍未就绪——卡在「阿里云短信企业资质未办 + 自托管实例出网 504」两项运维前置，
 * 「获取验证码」此刻仍会失败，开发阶段用邮箱登录推进；由 `PHONE_OTP_ENABLED` 开关控制（详见 REMAINING #10）。
 *
 ...
 * 注意短信通道尚未就绪（企业资质 + 实例出网 504，REMAINING #10），「获取验证码」此刻仍会失败，
 * 开发阶段请改用邮箱登录；通道就绪后即正常。置 false 可再次隐藏手机号入口、回退为仅邮箱 + Apple。
 */

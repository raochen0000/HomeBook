...
> 文档状态：已完成核心流程定义
> 负责人：产品组
> 技术实现：客户端采用 **React Native（Expo）+ TypeScript**；视觉规范见 `DESIGN.md`（实心材质 + Light / Night 两种模式，无玻璃 / 模糊），技术方案见 `TECH.md`。
...
| 引导页屏数       | 3 屏                              |
| 是否可跳过引导   | ✅ 可跳过                         |
| 登录方式         | 手机号验证码 + 第三方登录（MVP 第三方仅 Apple，详见 MVP.md §2.1） |
| 新用户家庭创建   | 自动创建单人家庭                  |
...
flowchart TD
    ...
    G --> H[页面包含: 二维码<br/>家庭名 + 户主信息<br/>有效期倒计时]
...
| 决策点       | 选择                             | 理由                     |
| ------------ | -------------------------------- | ------------------------ |
| 邀请方式     | 仅二维码（保存图片，App 内扫描） | 简单直接，无外部分享风险 |
| 二维码有效期 | 24 小时                          | 平衡安全性与易用性       |
| 是否限次数   | 不限                             | 户主可随时刷新           |
...
让被邀请人在 App 内扫码加入家庭，并清晰理解加入对当前家庭数据的影响。
...
flowchart TD
    ...
    E --> F[展示信息:<br/>家庭名 / 户主 / 现有成员数]
    F --> G{当前用户家庭状态}

    G -- 单人家庭 无记账 --> H1[直接进入加入确认]
    G -- 单人家庭 有记账 --> H2[警告: 加入后<br/>原家庭和数据将被删除]
    G -- 多人家庭 普通成员 --> H3[警告: 加入后<br/>将自动退出当前家庭]
    G -- 多人家庭 户主 --> H4[阻止加入<br/>需先转让或解散]

    H4 --> H4End([跳转流程 5：转让 / 解散<br/>完成后回到扫一扫重新加入])
...
| 户主直接加入新家庭 | 不允许，需先转让或解散         | 防止家庭无人管理         |
...
| 家庭已满          | 提示无法加入               |
| 已是该家庭成员    | 提示「你已经在这个家了」   |
| 当前是户主身份    | 阻止加入，引导先转让或解散；处理完成后需回到「扫一扫」重新扫码（原二维码若已过期，需户主重新生成） |
...
| 模块     | 内容                          | 形式          |
| -------- | ----------------------------- | ------------- |
| 顶部概览 | 当期收入、支出、结余          | 数字卡片      |
| 分类支出 | 各分类金额与占比              | 环形图 + 列表 |
| 成员贡献 | 各成员记账金额                | 横向条形图    |
| 趋势     | 近 7 日 / 近 30 日 / 近 12 月 | 折线图        |
| 收支对比 | 收入 vs 支出                  | 双柱图        |
...
✅ 流程 1-13 已全部完成定义（含本次补充的编辑/删除流水、分类管理、账号注销、通知体系）。
...
- §21 MVP 范围与排期建议
- ✅ §22 视觉设计规范 —— 已独立成文，详见 `DESIGN.md`（中性黑白骨架 + Light / Night 两种模式、收支柔和语义色、**实心材质（无玻璃 / 模糊）**、设计令牌与 AI 提示词模板）
